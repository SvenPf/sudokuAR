...
class App:
    ...
    def __init__(self, capture_device):
        ...
        self.perspective_transformer = PerspectiveTransformer(SUDOKU_GRID_HEIGHT, SUDOKU_GRID_WIDTH)
        self.umat_video_stream = UMatVideoStream(capture_device, SELECTION_RATE)
        self.num_classifier = NumberClassifier()
        self.grid_converter = GridConverter(self.num_classifier, SUDOKU_SHAPE, SUDOKU_GRID_HEIGHT, SUDOKU_GRID_WIDTH)
    ...
    def run(self):
        ...
        while not video.stopped:
            ...
            sudoku_grid_image = self.perspective_transformer.transform_image_perspective(frame, grid_location)

            # show converted frame
            cv2.imshow("Perspective Transformed", sudoku_grid_image)

            sudoku_grid_array = self.grid_converter.convert_image_to_array(sudoku_grid_image)

            if len(sudoku_grid_array) > 0:

                solved_sudoku_array = self.sudoku_solver.solve_array(sudoku_grid_array)

                if len(solved_sudoku_array) > 0:

                    solved_sudoku_image = self.grid_converter.convert_array_to_image(solved_sudoku_array - sudoku_grid_array)
                    wraped_solved_sudoku_image = self.perspective_transformer.inverse_transform_image_perspective(solved_sudoku_image, video.height, video.width)

                    cv2.imshow("Solution", cv2.addWeighted(frame, 0.8, wraped_solved_sudoku_image, 0.5, 0.0))
